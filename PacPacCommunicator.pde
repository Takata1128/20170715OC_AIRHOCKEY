...
class HandData{
  ...
  public boolean handState;                   // 1: pinched, 0: released
}
...
void initialize(){
  ...
  udpServer.setReceiveHandler("received");
...
void received(byte[] data, String hostIP, int portNo){
  ...
  for (int i = 0; i < MAX_HANDS; i++){    
    ...
    handDataArray[i].pinchX = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 16], data[(i * UDP_RECEIVE_DATA_SIZE) + 17], data[(i * UDP_RECEIVE_DATA_SIZE) + 18], data[(i * UDP_RECEIVE_DATA_SIZE) + 19]);       // arm X
    handDataArray[i].pinchY = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 20], data[(i * UDP_RECEIVE_DATA_SIZE) + 21], data[(i * UDP_RECEIVE_DATA_SIZE) + 22], data[(i * UDP_RECEIVE_DATA_SIZE) + 23]);       // arm Y    
    handDataArray[i].armX = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 24], data[(i * UDP_RECEIVE_DATA_SIZE) + 25], data[(i * UDP_RECEIVE_DATA_SIZE) + 26], data[(i * UDP_RECEIVE_DATA_SIZE) + 27]);     // pinch Z
    handDataArray[i].armY = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 28], data[(i * UDP_RECEIVE_DATA_SIZE) + 29], data[(i * UDP_RECEIVE_DATA_SIZE) + 30], data[(i * UDP_RECEIVE_DATA_SIZE) + 31]);     // pinch Y
    handDataArray[i].pinchOriginX = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 32], data[(i * UDP_RECEIVE_DATA_SIZE) + 33], data[(i * UDP_RECEIVE_DATA_SIZE) + 34], data[(i * UDP_RECEIVE_DATA_SIZE) + 35]);        // pinch originX
    handDataArray[i].pinchOriginY = byteArrayToFloat(data[(i * UDP_RECEIVE_DATA_SIZE) + 36], data[(i * UDP_RECEIVE_DATA_SIZE) + 37], data[(i * UDP_RECEIVE_DATA_SIZE) + 38], data[(i * UDP_RECEIVE_DATA_SIZE) + 39]);        // pinch originY
  }  
...
void keyPressed(){
  if (key == ' '){
    ...
    handDataArray[0].pinchX = mouseX;    
    handDataArray[0].pinchY = mouseY;    
  }
}
...
void drawHandMarker(){
  ...
  for ( int i = 0; i < MAX_HANDS; i++ ) {
    ...
    if ( handDataArray[i].id != 0 ) {    // idが0じゃないデータは有効なデータ
      ...
      /* 腕の位置 */
      ellipse(handDataArray[i].armX, handDataArray[i].armY, 200, 200);    
